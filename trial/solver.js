...
function solveSite(input) {
  ...
  const turn = industrial
    ? { ok: industrial.courtDepth >= 120, val: `卡车回转场 ${Math.round(industrial.courtDepth)}ft ≥ 53呎拖车需 120ft` }
    : { ok: 24 >= (TURN_STD[ang] || 24) - 0.5, val: `车道 24ft ≥ ${ang}° 标准 ${TURN_STD[ang] || 24}ft · 双向回转净空足` };

  // 7. zoning compliance (Site Intelligence pass/fail)
  const compliance = [
    { k: 'FAR 容积', ok: far <= p.maxFAR + 1e-6, val: `${far.toFixed(2)} / 上限 ${p.maxFAR}` },
    ...
    { k: '停车 Parking', ok: parkingProvided >= parkingRequired,
      val: deckType ? `${parkingProvided} / 需 ${parkingRequired}（${isWrap ? '环绕核心 ' : ''}地上${levelsAbove}+地下${levelsBelow}层 × ${parkingPerFloor}/层 × ${structEff}%）` : `${parkingProvided} / 需 ${parkingRequired}` },
  ];
  ...
  if (industrial)
    compliance.push({ k: '物流规模 Logistics', ok: true, info: true, val: `${industrial.dockCount} 月台门 · ${industrial.trailerCount} 拖车位 · ${industrial.dockType === 'cross' ? '双面对流 Cross-dock' : '单面 Single-dock'}` });
  if (hotel)
    compliance.push({ k: '客房数 Keys', ok: true, info: true, val: `${keys} 间 · 双载走廊 ${floors}F` });
  if (retail)
    compliance.push({ k: '零售规模 Retail', ok: true, info: true, val: `主力店+${retail.pads.length} pad 外带店 · GLA ${Math.round(retail.gla).toLocaleString()} SF` });
  if (datacenter)
    compliance.push({ k: '机房规模 Data hall', ok: true, info: true, val: `资料机房 ${Math.round(gfa).toLocaleString()} SF · ${floors} 层 · 含机电中庭＋变电站` });
  if (groundRetail)
    compliance.push({ k: '复合用途 Mixed-use', ok: true, info: true, val: `底层零售 ${Math.round(retailGLA).toLocaleString()} SF ＋ 住宅 ${units} 户（${resiFloors}F）` });
  compliance.push({ k: '回转/车道 Turning', ok: turn.ok, val: turn.val });
  if (coreInfo)
    compliance.push({ k: '核心 Cores', ok: true, info: true, val: `${coreInfo.stairs} 逃生梯 ＋ ${coreInfo.elevators} 电梯（自动配置）` });
