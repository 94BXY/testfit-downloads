...
const S = {
  ...
  tool: 'select',
  params: { angle: 90, stallW: 8.20, stallD: 17.39, aisle: 19.69, setback: 9.84, orient: 'auto', height: 35, oneway: false, greenBuffer: 0, maxRun: 0, maxRunGap: 9, compactW: 7.5, access: 'multi' },
  ...
  geo: { lat0: 31.2304, lng0: 121.4737, set: false, firstOpen: true },  // default: 上海浦东
  ...
  layers: {                 // per-category visibility + lock (object-tree eye / lock)
    ...
    trees:    { vis: true },
    flow:     { vis: false },        // 动线体检 congestion heat-map layer (off by default)
    earthwork:{ vis: false },        // 挖填方 cut/fill gradation layer (off by default)
    unitfit:  { vis: true },         // 户型平面 unit-fit floor plan (on by default — the signature view)
  },
  bgImg: null,               // {img, x, y, w, h, opacity} background image for tracing
  bgImgDrag: null,           // {startX,startY,origX,origY} drag state
};

const COLORS = {
  standard:'#3b82f6', compact:'#22c55e', ada:'#1d4ed8', ev:'#10b981', trailer:'#f59e0b', moto:'#a855f7',
};
const LABELS = { standard:'标准 Standard', compact:'小型 Compact', ada:'♿ 无障碍 ADA', ev:'⚡ EV 充电', trailer:'拖车 Trailer', moto:'🏍️ 机车 Motorcycle' };
...
const USE_LABEL = { residential: '住宅', office: '办公', retail: '零售', hotel: '旅馆', industrial: '工业/仓储', datacenter: '资料中心' };
...
function updateFinancials(){    // refresh the pro-forma readout (massing-driven in parking mode, site solve in site mode)
  ...
  if (!fin || !(fin.totalCost > 0)) { el.textContent = S.mode === 'site' ? '按「自动配置建案」后显示财务' : '画建筑量体＋设定参数后自动试算开发财务'; return; }
  el.innerHTML = `总开发成本 <b style="color:#e2e8f0">$${Math.round(fin.totalCost).toLocaleString()}</b>　·　年 NOI <b style="color:#e2e8f0">$${Math.round(fin.noi).toLocaleString()}</b><br>殖利率 Yield <b style="color:#4ade80">${fin.yieldOnCost.toFixed(1)}%</b>${fin.irr != null ? `　·　IRR <b style="color:#4ade80">${fin.irr.toFixed(1)}%</b>` : ''}`;
}
...
const REGIONS = {
  // ——— 中国大陆 GB 标准 ———
  // 停车：GB 50220-2018《城市道路交通设施设计规范》/ JGJ 100-2015《车库建筑设计规范》
  //   小型车 2.5×5.3m，微型车 2.2×4.8m；车道宽 ≥6m（双向）/ ≥3.5m（单向）
  //   垂直式车格 5.3m深 + 6m道 = 11.3m柱网间距常见 8.1~8.4m 开间
  // 退缩：GB 50180-2018《城市居住区规划设计标准》/ 各地规划条例
  //   前退缩 ≥5m（主干道），侧面 ≥3m，后面 ≥5m（高层需加大）
  // 容积率/覆盖率：各地控规不同，以下为中等城市住宅参考值
  cn: { unit: 'metric', stallW: 8.20, stallD: 17.39, aisle: 19.69, setback: 9.84,
        site: { floorH: 10.5, maxFAR: 3.0, maxHeight: 100, maxCov: 35, maxDUA: 0, sbF: 16.4, sbS: 6.6, sbR: 9.8, parkRatio: 1.0 } },
...
const LEN_LABELS = {
  pW: '车格宽 Stall W', pD: '车格深 Stall D', pA: '车道宽 Aisle', pS: '退缩 Setback', pH: '建筑高度 Height',
  sFloorH: '楼层高 Floor-to-floor', zHeight: '最大高度 Height', bHeight: '建筑高度 Height', cW: '🚗 Compact 车格宽',
};
...
function refreshUnitLabels() {
  ...
  if (sbLab) sbLab.textContent = `退缩 前/侧/后 (${lu})`;
  ...
  setL('mArea', `基地面积 (${bu})`);
  setL('mEff', `${au} / 车位`);
  setL('mRatio', U.metric() ? '车位比 /100 m²' : '车位比 /1000 SF');
  setL('sGFA', `总楼地板 GFA (${au})`);
  ...
  if (uh) uh.textContent = `坪数 ${au}`;
  const gfaLab = $('#pGFA') && $('#pGFA').closest('.field').querySelector('label');
  if (gfaLab) gfaLab.textContent = `建筑 GFA (${au})`;
}
...
function applyRegion(r) {
  ...
  refreshUnitLabels();
  toast(`已套用 ${({ cn: '国标', us: '美规', jp: '日本', eu: '欧洲' })[r]} 参考预设（可再手动调整）`);
...
async function fetchContextBuildings() {
  if (!S.mapMode || !S.map) { toast('请先开启「地图」底图，再载入周边建物'); return; }
  ...
  try {
    ...
    toast(bldgs.length ? `已载入 ${bldgs.length} 栋周边既有建物 — 切「3D 量体」看 context` : '这个范围 OSM 没有建物资料');
  } catch (e) { toast('周边建物载入失败（需网路；OSM 服务忙碌时稍后再试）'); }
}
...
function draw() {
  ...
  // background image (imported blueprint / site plan)
  if (S.bgImg && S.bgImg.img) {
    const bi = S.bgImg;
    ctx.save();
    ctx.globalAlpha = bi.opacity != null ? bi.opacity : 0.5;
    ctx.drawImage(bi.img, bi.x, bi.y, bi.w, bi.h);
    ctx.globalAlpha = 1;
    // draw border around image
    ctx.strokeStyle = 'rgba(56,189,248,.6)'; ctx.lineWidth = 1.5;
    ctx.setLineDash([6,4]); ctx.strokeRect(bi.x, bi.y, bi.w, bi.h); ctx.setLineDash([]);
    // corner handles
    const hs = 6;
    ctx.fillStyle = '#38bdf8';
    [[bi.x, bi.y], [bi.x+bi.w, bi.y], [bi.x, bi.y+bi.h], [bi.x+bi.w, bi.y+bi.h]].forEach(([hx,hy]) => {
  ...
  if (park && S.layers.parking.vis) {
    ...
    //     roads never break where segments meet (fixes "用线段联系会断开道路"). (2) round-cap STROKE each
  ...
  if (S.mode === 'site' && S.site && S.layers.building.vis) {
    ...
    if (sub && sub.units && sub.units.length) {
      ...
      const tlabel = { townhome: '户连栋', detached: '户独栋', cottage: '户小宅' }[sub.subType] || '户';
      ...
      labelPoly(S.site.footprint && S.site.footprint.length >= 3 ? S.site.footprint : S.boundary, `${S.site.garden.rows} 栋花园公寓 · ${S.site.units} 户 · ${S.site.floors}F`, '#e2e8f0');
      ...
      drawUnitPlan();
      labelPoly(tw.plate, `塔楼 ${S.site.units} 户 · ${S.site.floors}F · 基座${tw.podiumLevels}层车库`, '#e2e8f0');
      ...
      drawUnitPlan();
      labelPoly(S.site.footprint, `环绕 · ${S.site.units} 户 · ${S.site.floors}F`, '#e2e8f0');
      ...
      labelPoly(S.site.footprint, `仓储 ${ind.dockType === 'cross' ? '双面对流' : '单面'}卸货 · ${ind.dockCount} 门 · ${Math.round(S.site.gfa).toLocaleString()} SF`, '#e2e8f0');
      ...
      labelPoly(rt.anchor, `零售中心 · ${rt.pads.length} pad 外带店 · GLA ${Math.round(S.site.gfa).toLocaleString()} SF`, '#e2e8f0');
      ...
      labelPoly(dc.hall, `资料机房 · ${S.site.floors}F · ${Math.round(S.site.gfa).toLocaleString()} SF`, '#e2e8f0');
      const c = PS.centroid(dc.mechYard), sc = toScreen(c); ctx.fillStyle = 'rgba(120,53,15,.95)'; ctx.font = 'bold 10px system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('机电中庭', sc.x, sc.y);
      ...
      drawUnitPlan();
      const lbl = S.site.hotel ? `旅馆 ${S.site.keys} 房 · ${S.site.floors}F`
        : (S.site.unitPlan ? `${S.site.units} 户 · ${S.site.floors}F · ${Math.round(S.site.gfa).toLocaleString()} SF`
...
function addManualAisle(line) {
  const park = activePark();
  if (!park || !park.aisles) { toast('请先「自动排车位」，再加场内道路'); return; }
  ...
  draw(); commit();
  toast(`已新增场内道路（＋${fresh.length} 车位，已接入动线网）`);
}
...
// so 单边停 / 移除 survive a re-solve AND can be undone (clear the override → re-pack restores the stalls).
...
function setAisleEdit(i, op) {     // record (or clear, op=null) an override for an aisle, then apply/restore
  ...
  if (op === 'single' || op === 'single2') {          // apply now to the live solution (instant); keep the aisle selected
    if (park.spines && park.spines[i] && park.spines[i].kind === 'aisle' && park.spines[i].sides) { park.spines[i].sides = 0; retileSpine(i, true); }   // restore BOTH rows first so the side-filter has both sides to pick from (lets 翻面 re-flip, and re-单边停 after a flip, instead of filtering an already-emptied side to 0)
    ...
    toast(op === 'remove' ? '已移除这条场内道路与车位' : '已还原双边停车');
  }
}
...
function flipAisle(i) {              // single-loaded 场内道路 → swap which side keeps its stalls (TestFit "flip")
  const park = activePark(); if (!park || !park.aisles[i]) return;
  const cur = (S.aisleEdits || {})[aisleKey(park.aisles[i].poly)];
  if (cur !== 'single' && cur !== 'single2') { toast('先按「单边停」，才能翻到另一边'); return; }
...
function drawEntrance(e) {
  ...
  const label = type === 'in' ? '进' : type === 'out' ? '出' : '进出';
  // driveway / curb-cut stub + direction arrowhead(s): 进 in, 出 out, 进出 both
...
function drawDraft() {
  ...
  if (S.draftKind === 'road' && S.draft.length >= 2) {                  // hint: how to finish an open road
    ...
    ctx.fillText('双击或 Enter 完成道路', last.x + 10, last.y - 6);
  }
...
function computeEarthwork() {
  ...
  if (S.boundary.length < 3) { out.textContent = '先画基地。'; S._ewCache = null; return; }
  const NW = +$('#eNW').value, NE = +$('#eNE').value, SW = +$('#eSW').value, SE = +$('#eSE').value;
  if (!NW && !NE && !SW && !SE) { out.textContent = '输入四角标高后自动算挖填量与土方成本。'; S._ewCache = null; return; }
  ...
  out.innerHTML = `挖 <b>${fmt(cutY)}</b> yd³ ・ 填 <b>${fmt(fillY)}</b> yd³ ・ ${net >= 0 ? '外运 export' : '进土 import'} <b>${fmt(Math.abs(net))}</b> yd³<br>土方成本约 <b>$${fmt(cost)}</b> ・ 平衡整平高 ≈ <b>${balance.toFixed(1)} ft</b>（挖填相抵、免外运）`;
...
function computeErrors() {
  ...
  if (!area) { E.push({ lv: 'warn', m: '尚未画出基地边界' }); return E; }
  if (S.mode === 'site') {
    if (!S.site) E.push({ lv: 'warn', m: '尚未配置建案（按「自动配置建案」）' });
    else {
      S.site.compliance.filter(c => !c.ok).forEach(c => E.push({ lv: 'error', m: `法规不符：${c.k} = ${c.val}` }));
      if (S.site.densityCapped) E.push({ lv: 'warn', m: '户数受密度上限（DU/ac）限制' });
    }
    const sum = +$('#uxStudioP').value + +$('#ux1P').value + +$('#ux2P').value + +$('#ux3P').value;
    if (Math.abs(sum - 100) > 0.5) E.push({ lv: 'warn', m: `单元配比合计 ${sum}%（非 100%）` });
  } else {
    const sol = S.solution;
    if (!sol) E.push({ lv: 'warn', m: '尚未排车位（按「自动排车位」）' });
    else {
      if (sol.unreachable) E.push({ lv: 'error', m: `${sol.unreachable} 个车位无车道可达（已移除，建议补出入口）` });
      if (!S.entrances.length) E.push({ lv: 'warn', m: '尚未设出入口（动线无进出点）' });
      if (S.opts.target > 0 && sol.stalls.length < S.opts.target) E.push({ lv: 'error', m: `未达目标车位：${sol.stalls.length} / ${S.opts.target}（差 ${S.opts.target - sol.stalls.length}）` });
    }
  }
  return E;
}
function showErrors() {
  ...
  const html = E.length
    ...
    : '<div class="cc"><span class="badge ok">✓</span><span class="nm">没有问题，方案通过所有检查。</span></div>';
  const disc = '<div style="margin-top:12px;padding:9px 11px;border:1px solid var(--line);border-radius:7px;font-size:11px;color:var(--muted);line-height:1.7;">'
    + '⚠️ <b>可行性估算工具</b>：本工具<b>尚未检核</b>消防车道（净宽/回转/到建筑距离）、死巷回转空间（hammerhead）、闸口排队堆叠长度、无障碍通道几何、坡道/净高、机车位。'
    + '输出供量体与车位数估算，<b>不可作为施工图或送审依据</b>，实际设计仍须依当地法规与专业技师检核。</div>';
  openModal(`检查 Errors — ${E.length} 项`, html + disc);
}
...
function updateTabBar() {
  ...
  const siteKv = [['面积', `${U.big(area).toFixed(2)} ${U.bu()}`], ['', `${Math.round(U.A(area)).toLocaleString()} ${U.au()}`]];
  if (S.mode === 'site' && S.site) siteKv.push(['FAR', S.site.far.toFixed(2)], ['建蔽', S.site.coverage.toFixed(0) + '%']);
  let html = col('SITE 基地', siteKv);
  html += col('PARKING 停车', [['车位', stalls.toLocaleString()],
    ['单格', `${Math.round(U.A(moduleArea))} ${U.au()}`], ['覆盖', cover + '%']]);
  if (S.mode !== 'site' && park) {            // parking mode — stall-type & target breakdown
    ...
    html += col('STALLS 车种', [['标准', cc.standard], ['⚡EV', cc.ev], ['♿ADA', cc.ada],
      tg ? ['🎯需求', `${stalls}/${tg}`] : ['小型', cc.compact]]);
  }
  if (S.mode === 'site' && S.site) {
    const s = S.site;
    html += col('BUILDING 建筑', [['GFA', Math.round(U.A(s.gfa)).toLocaleString()],
      [s.residential ? '户数' : 'NRSF', s.residential ? s.units : Math.round(U.A(s.nrsf)).toLocaleString()],
      ['停车', `${s.parkingProvided}/${s.parkingRequired}`], ['Yield', s.fin.yieldOnCost.toFixed(1) + '%']]);
    if (s.residential) {                      // unit-level tabulation: DU/AC · Beds · Baths
      ...
      html += col('UNITS 户量', [['DU/ac', (s.units / s.acres).toFixed(1)],
  ...
  html += `<div class="col" id="tabErr" style="cursor:pointer;"><h5>${errs.length ? '⚠' : '✅'} 检查 Errors</h5>
    <div class="kv"><span class="k">问题</span><span class="v" style="color:${errColor}">${errs.length || 'OK'}</span></div>
    ${errs.length ? `<div class="kv" style="font-size:10px;color:var(--muted);">${esc(errs[0].m.slice(0, 14))}…</div>` : '<div class="kv" style="font-size:10px;color:var(--muted);">点看详情</div>'}</div>`;
...
function draw3D() {
  ...
  if (S.boundary.length < 3) {
    ...
    ctx.fillText('切到 2D 画出基地后，这里会显示 3D 量体', cv._w / 2, cv._h / 2);
    return;
  }
  const c = PS.centroid(S.boundary); S.iso.cx = c.x; S.iso.cy = c.y;
  ...
  if (S.mode === 'site' && S.site) {
    ...
    if (sub && sub.units && sub.units.length) {
      ...
      S.site.garden.bars.forEach((b, i) => pushBox(faces, b, S.site.height, '#0ea5e9', i === 0 ? `${S.site.garden.rows}栋 ${S.site.floors}F` : null, true, null, 0));
    } else if (S.site.tower && S.site.tower.podium) {
      ...
      pushBox(faces, S.site.footprint, S.site.height, '#475569', `仓储 ${ind.dockCount}门`, true, null, 0);
    } else if (S.site.retail) {
      ...
      pushBox(faces, dc.hall, S.site.height, '#334155', `机房 ${S.site.floors}F`, true, null, 0);
    } else if (S.site.footprint && S.site.footprint.length >= 3) {
  ...
  const hud3d = (S.mode === 'site' && S.site && S.site.structured)
    ? `3D 量体　🏢 结构车库 地上${S.site.levelsAbove}+地下${S.site.levelsBelow}层 · 共 ${S.site.parkingProvided} 车位（每层 ${S.site.parkingPerFloor}）`
    : `3D 量体　建筑高度 ${S.params.height} ft　·　${S.solution ? S.solution.stalls.length + ' 车位' : ''}`;
...
cv.addEventListener('pointerdown', e => {
  const w = evtWorld(e);
  // if clicking on background image, let the mousedown handler handle it (drag/move)
  if (S.bgImg && e.button === 0 && !S.mapMode) {
    const r = cv.getBoundingClientRect(), mx = e.clientX - r.left, my = e.clientY - r.top;
    const bi = S.bgImg;
    if (mx >= bi.x && mx <= bi.x + bi.w && my >= bi.y && my <= bi.y + bi.h) {
      // only intercept if tool is select/pan (drawing tools should still work on top of image)
  ...
  if (S.tool === 'entrance') {
    ...
    toast(S.params.access === 'single' ? '单一出入口：已设定唯一闸口' : '已放出入口 — 选取可拖曳移动、双击改进/出、右键删除');
  ...
  if (S.tool === 'subdivide') {           // two clicks define a cut line
    const sp = snap(w);
    if (!S.splitPt) { S.splitPt = sp; toast('再点第二点，画一条横跨基地的切割线'); draw(); }
  ...
  if (S.tool === 'stall') {               // manually drop one parking stall at the click (override the auto layout)
    ...
    if (!sol || !sol.stalls) { toast('请先按「自动排车位」排一次'); return; }
  ...
  if (S.tool === 'measure') {             // two clicks → a dimension annotation
    const sp = snap(w);
    if (!S.measureStart) { S.measureStart = sp; toast('再点第二点量出距离（右键清除全部）'); draw(); }
  ...
  if (S.tool === 'select') {
    ...
    { if (connectorAt(w) >= 0) { S.selAisle = null; hideAislePopup(); toast('场内道路：拖两端的点可调整这条线，车位会跟著重贴'); draw(); return; } }
...
cv.addEventListener('contextmenu', e => {
  e.preventDefault();
  const w = evtWorld(e);
  if (S.tool === 'measure') { S.measures = []; S.measureStart = null; draw(); toast('已清除量测'); return; }
  if (S.draft) { finishDraft(); return; }
  if (S.tool === 'select' && pickable('entrance')) {
    const eh = entranceAt(w);
    if (eh) { S.entrances.splice(S.entrances.indexOf(eh), 1); S.selEntrance = null; draw(); toast('已删除出入口'); resolveActive(); return; }
  }
  if (S.tool === 'select') {                       // right-click a 场外道路 → delete it
    const rh = roadVertexAt(w) || roadBodyAt(w);
    if (rh) { S.roadLines.splice(rh.ri, 1); S.selRoad = null; hideRoadPopup(); roadChanged(); toast('已删除场外道路'); return; }
  }
  if (S.tool === 'select' && pickable('parking')) {   // right-click a 场内道路 → open its manual menu (单边停/翻面/移除), TestFit-style
    const ai = aisleAt(w);
...
cv.addEventListener('dblclick', e => {
  ...
  if (eh) {
    ...
    draw(); toast('出入口：' + (eh.type === 'in' ? '只进 ▸' : eh.type === 'out' ? '只出 ◂' : '进出 ⇄'));
  ...
  if (park && park.spines) {
    ...
    if (sn && park.spines[sn.si] && park.spines[sn.si].kind === 'aisle') {
      ...
      if (sn.ni > 0 && sn.ni < ln.length - 1) { ln.splice(sn.ni, 1); retileSpine(sn.si, true); reconnectNetwork(); S.selAisle = sn.si; (S.mode === 'site' ? updateSiteMetrics : updateMetrics)(); draw(); commit(); toast('已移除弯折点'); return; }
    }
    const sb = spineBodyAt(w);
    if (sb >= 0 && park.spines[sb] && park.spines[sb].kind === 'aisle' && insertSpineNode(sb, w)) {
      S.selAisle = sb; (S.mode === 'site' ? updateSiteMetrics : updateMetrics)(); draw(); commit(); toast('已加弯折点 — 拖动黄点可弯曲车道'); return;
    }
  }
...
function finishDraft() {
  ...
  if (S.draftKind === 'boundary') { S.boundary = poly; S.solution = null; S.edgeSetback = {}; S.selEdge = null; msg = '基地完成，按「自动排车位」'; }
    ...
    if (host) { (host.voids || (host.voids = [])).push(poly); S.selBuilding = host; msg = '已在建筑上挖出中庭 Void'; }
    else {
      const nb = makeBuilding(poly); S.buildings.push(nb); S.selBuilding = nb;
      msg = (S.boundary.length >= 3 && !PS.polyOverlap(poly, S.boundary)) ? '⚠️ 建筑画在基地范围外，对排版无影响' : '已新增建筑（右侧可改颜色/高度/屋顶）';
    }
  }
  else if (S.draftKind === 'obstacle') {
    S.obstacles.push(poly);
    if (S.boundary.length >= 3 && !PS.polyOverlap(poly, S.boundary)) msg = '⚠️ 障碍画在基地范围外，对排版无影响';
  }
...
window.addEventListener('keydown', e => {
  ...
  if (e.key === 'Delete' || e.key === 'Backspace') {
    if (S.selEntrance) {
      ...
      S.roadLines.splice(S.selRoad, 1); S.selRoad = null; hideRoadPopup(); roadChanged(); toast('已删除道路');
    } else if (S.selAisle != null) {
      removeAisle(S.selAisle);
    } else if (S.selObstacle != null && S.obstacles[S.selObstacle]) {
      S.obstacles.splice(S.selObstacle, 1); S.selObstacle = null; resolveActive(); draw(); toast('已删除障碍');
...
function setTool(t) {
  ...
  const names = { select:'选取/编辑', boundary:'画基地', building:'画建筑', obstacle:'画障碍', road:'画场外道路', aisle:'画场内道路', parkzone:'画停车区', stall:'加车位', core:'放核心', entrance:'放出入口', subdivide:'切割子地', measure:'量测距离', pan:'平移' };
...
function updateMetrics() {
  ...
  $('#stArea').textContent = siteArea
    ? `面积：${Math.round(U.A(siteArea)).toLocaleString()} ${U.au()} (${U.big(siteArea).toFixed(2)} ${U.bu()})` : '面积：—';
  ...
  if (required > 0) {
    ...
    dBadge.textContent = ok ? `✓ 停车充足　${total} / 需 ${required}　(多 ${diff})` : `✗ 停车不足　${total} / 需 ${required}　(缺 ${-diff})`;
  ...
  if (counts.ada) {
    ...
    note.className = 'hint';
    note.textContent = `法规最低 ADA：${req}（含 van 无障碍 ${Math.max(1, Math.ceil(counts.ada/6))}）`;
    bd.appendChild(note);
  }
  if (sol && S.entrances.length) {
    ...
    cn.textContent = sol.unreachable
      ? `🚗 动线：${sol.unreachable} 个车位无车道可达，已移除（建议补出入口）`
      : `🚗 动线：${S.entrances.length} 个出入口，全部车位皆可达 ✓`;
    bd.appendChild(cn);
  }
  if (S.opts.target > 0 && sol) {
    const ok = total >= S.opts.target, d = document.createElement('div');
    d.id = 'targetBadge'; d.className = ok ? 'pass' : 'fail';
    d.textContent = ok ? `🎯 达成目标 ${total} / ${S.opts.target} ✓` : `🎯 未达目标：${total} / ${S.opts.target}（差 ${S.opts.target - total}）`;
    bd.appendChild(d);
  }
  if (!any) { bd.innerHTML = '<div class="hint">尚未排车位 — 画好基地后按「⚡ 自动排车位」。</div>'; }
}

function buildLegend() {
  const L = $('#legend'); L.innerHTML = '';
  const items = [
    ['standard','标准'], ['compact','小型'], ['ev','EV'], ['ada','ADA'], ['moto','机车'], ['trailer','拖车'],
  ];
  ...
  extra.innerHTML = `<span class="sw" style="background:rgba(203,213,225,.4)"></span>场内道路`;
  L.appendChild(extra);
...
function renderSchemes() {
  ...
  if (!arr.length) { list.innerHTML = '<div class="hint">尚无储存方案。</div>'; return; }
  arr.forEach((sc, i) => {
...
function saveScheme() {
  ...
  let summary;
  if (S.mode === 'site') {
    summary = S.site ? `${S.site.residential ? S.site.units + ' 户' : Math.round(S.site.gfa).toLocaleString() + ' SF'} · ${S.site.floors}F` : '建案';
  } else {
    const stalls = S.solution ? S.solution.stalls.length : 0, req = parkingDemand(), fin = massingFinancials();
    summary = `${stalls} 车位${req ? `/需${req}` : ''} · ${S.params.angle}°${fin && fin.yieldOnCost ? ` · Yield ${fin.yieldOnCost.toFixed(1)}%` : ''}`;
  }
  const snap = {
...
function loadScheme(sc) { deserialize(sc.data); toast('已载入：' + sc.name); }
...
function serialize() {
  ...
  // save bgImg as data URL (Image object can't be serialized)
  if (S.bgImg && S.bgImg._dataUrl) {
    s.bgImg = { dataUrl: S.bgImg._dataUrl, x: S.bgImg.x, y: S.bgImg.y, w: S.bgImg.w, h: S.bgImg.h, opacity: S.bgImg.opacity };
  }
  return s;
}
function deserialize(d) {
  ...
  // restore bgImg from data URL
  if (d.bgImg && d.bgImg.dataUrl) {
    const img = new Image();
    img.onload = () => { S.bgImg = { img, x: d.bgImg.x, y: d.bgImg.y, w: d.bgImg.w, h: d.bgImg.h, opacity: d.bgImg.opacity, _dataUrl: d.bgImg.dataUrl }; draw(); };
    img.src = d.bgImg.dataUrl;
  } else {
    S.bgImg = null;
  }
...
function undo() { if (S.hIdx > 0) { S.hIdx--; applyHistory(); toast('↶ 已复原 Undo'); } }
...
function exportOBJ() {
  ...
  toast('已汇出 3D 模型 .obj（可汇入 SketchUp / Blender / Rhino）');
}
...
function exportGLTF() {
  ...
  if (!pos.length) { toast('没有可汇出的几何'); return; }
  ...
  toast('已汇出 3D 模型 .glb（Blender / 网页 3D / Revit 可汇入）');
}
...
/* ------------------------------ wiring ----------------------------------- */
$('#btnSolve').onclick = () => toast('🔒 试用版不支持自动规划，请购买完整版');
$('#btnSample').onclick = () => {
...
// rows fit → 扩容. Two-way is wider for two-direction circulation.
...
function applyAngleAisle() {
  // scale the angle-based aisle to the current region's standard, so China stays ~6 m
...
document.querySelectorAll('#onewaySeg button').forEach(b => b.onclick = () => {
  ...
  if (S.boundary.length >= 3) doSolve();
  toast(S.params.oneway ? '单向车道：车道较窄、可塞更多车位' : '双向车道');
});
...
// trial: parameter changes don't auto-solve
['#pW','#pD','#pA','#pS','#pOrient','#pGreen','#pMaxRun','#pMaxGap'].forEach(s => $(s).addEventListener('change', () => { S.gridShift = null; S.aisleEdits = null; /* trial: no auto-solve */ }));
...
$('#btnRowAxis').onclick = () => {
  ...
  if (S.boundary.length < 3) { toast('请先画出基地边界再换动线'); return; }
...
document.querySelectorAll('#accessSeg button').forEach(b => b.onclick = () => {
  ...
  if (S.solution) doSolve();
  toast({ open: '开放式：临路面开放、不设闸口（保留全部车位）', single: '单一出入口：只用一个闸口，其余车位需从它连通', multi: '多出入口：可设多个闸口' }[S.params.access]);
});
...
$('#impJSON').onchange = e => {
  ...
  rd.onload = () => { try { deserialize(JSON.parse(rd.result)); toast('已汇入'); } catch { toast('档案格式错误'); } };
  rd.readAsText(f);
};
...
const UNIT_LABEL = { studio: 'Studio 套房', '1br': '1-Bed 一房', '2br': '2-Bed 二房', '3br': '3-Bed 三房',
  townhome: '连栋 Townhome', detached: '独栋 Detached', cottage: '小宅 Cottage/ADU' };
...
function setMode(mode) {
  ...
  $('#btnSolve').lastChild.textContent = mode === 'site' ? '自动配置建案' : '自动排车位';
  $('#stTool').textContent = '模式：' + (mode === 'site' ? '建案规划' : '停车场');
...
function updateSiteMetrics() {
  ...
  if (s) {
    ...
    if (s.industrial) {                                    // warehouse: dock doors + trailer stalls + clear height
      ...
      [['🚛 卸货月台门 Dock doors', ind.dockCount + ' 门'],
       ['🚚 拖车位 Trailer stalls', ind.trailerCount + ' 位'],
       ['📐 净高 Clear height', Math.round(U.L(s.height)) + ' ' + U.lu()]].forEach(([label, val]) => {
        const row = document.createElement('div'); row.className = 'brow';
    ...
    if (s.hotel) {                                          // hotel: room keys
      ...
      const nm = document.createElement('span'); nm.className = 'nm'; nm.textContent = '🛎️ 客房 Keys · 双载走廊';
      const ct = document.createElement('span'); ct.className = 'ct'; ct.textContent = s.keys + ' 间';
      row.append(sw, nm, ct); bd.appendChild(row);
    }
    const pr = document.createElement('div'); pr.className = 'hint'; pr.style.marginTop = '4px';
    const parkTxt = s.isWrap
      ? `🏟️ 环绕车库 地上${s.levelsAbove}+地下${s.levelsBelow}层 × ${s.parkingPerFloor}/层（核心）= 提供 ${s.parkingProvided} / 需 ${s.parkingRequired}`
      : s.structured
      ? `🏢 结构车库 地上${s.levelsAbove}+地下${s.levelsBelow}层 × ${s.parkingPerFloor}/层 × ${s.structEff}% = 提供 ${s.parkingProvided} / 需 ${s.parkingRequired}`
      : `🅿️ 地面停车 提供 ${s.parkingProvided} / 需 ${s.parkingRequired}`;
    pr.textContent = `${parkTxt}　·　土建 $${fmt(s.fin.totalCost)}　·　NOI $${fmt(s.fin.noi)}/年`;
    bd.appendChild(pr);
  } else bd.innerHTML = '<div class="hint">画好基地后按「自动配置建案」。</div>';
  renderCompliance(s);
}

function renderCompliance(s) {
  const box = $('#complianceList'); box.innerHTML = '';
  if (!s) { box.innerHTML = '<div class="hint">画好基地、按「自动配置建案」后显示。</div>'; return; }
  ...
  score.textContent = pass === total ? `✓ 全数通过 (${pass}/${total})` : `✕ ${total - pass} 项不符法规 (${pass}/${total} 通过)`;
  ...
  if (s.densityCapped) {
    ...
    h.textContent = '※ 户数受密度上限（DU/acre）限制，未用满容积。'; box.appendChild(h);
  }
  if (s.parkCapped) {
    const h = document.createElement('div'); h.className = 'hint'; h.style.marginTop = '6px';
    h.textContent = '※ 花园公寓户数受地面停车容量限制（低层多栋的密度限制因子）。'; box.appendChild(h);
  }
}

function updateUxSum() {
  const sum = +$('#uxStudioP').value + +$('#ux1P').value + +$('#ux2P').value + +$('#ux3P').value;
  $('#uxSum').textContent = '占比合计：' + sum + '%' + (sum !== 100 ? '（非 100% 会依比例换算）' : '');
}
...
/* ----------------------- 范例展示 demo gallery (one-click scenarios) ----------------------- */
const DEMO_PARCELS = {
...
const DEMOS = [
  { key: 'pk-surf',  g: '停车型态', label: '地面停车',           parcel: 'sample', use: 'multifamily', park: 'surface' },
  { key: 'pk-above', g: '停车型态', label: '结构车库·地上3层',   parcel: 'sample', use: 'multifamily', park: 'structured', above: 3, below: 0 },
  { key: 'pk-below', g: '停车型态', label: '结构车库·地下3层',   parcel: 'sample', use: 'multifamily', park: 'structured', above: 0, below: 3 },
  { key: 'pk-mix',   g: '停车型态', label: '地上2+地下2 混合',    parcel: 'sample', use: 'multifamily', park: 'structured', above: 2, below: 2 },
  { key: 'pk-wrap',  g: '停车型态', label: '环绕车库 Wrap',       parcel: 'sample', use: 'multifamily', park: 'wrap', above: 5, below: 0 },
  { key: 'u-mf',  g: '建筑用途', label: '多户住宅 Multifamily',   parcel: 'sample', use: 'multifamily',  park: 'structured', above: 3, below: 0 },
  { key: 'u-twr', g: '建筑用途', label: '高层塔楼 Tower（点式高楼）', parcel: 'big', use: 'tower', park: 'structured', above: 2, below: 0, far: 8, cov: 60, hgtFt: 300 },
  { key: 'u-sf',  g: '建筑用途', label: '连栋 Townhome', parcel: 'sample', use: 'singlefamily', park: 'surface', far: 0.55, cov: 35, sub: 'townhome' },
  { key: 'u-sf2', g: '建筑用途', label: '独栋 Detached（大地坪+院）', parcel: 'big', use: 'singlefamily', park: 'surface', far: 0.55, cov: 35, sub: 'detached' },
  { key: 'u-sf3', g: '建筑用途', label: '小宅 Cottage / ADU', parcel: 'sample', use: 'singlefamily', park: 'surface', far: 0.55, cov: 35, sub: 'cottage' },
  { key: 'u-grd', g: '建筑用途', label: '花园公寓 Garden（低层多栋）', parcel: 'big', use: 'garden', park: 'surface', far: 1.0, cov: 45 },
  { key: 'u-mix', g: '建筑用途', label: '复合用途 Mixed-use',     parcel: 'sample', use: 'mixeduse',     park: 'structured', above: 2, below: 1 },
  { key: 'u-off', g: '建筑用途', label: '办公 Office',            parcel: 'sample', use: 'office',       park: 'structured', above: 3, below: 1 },
  { key: 'u-ret', g: '建筑用途', label: '零售中心 Retail（主力店+pad）', parcel: 'big', use: 'retail',  park: 'surface', far: 0.4, cov: 30 },
  { key: 'u-hot', g: '建筑用途', label: '旅馆 Hotel',             parcel: 'big',    use: 'hotel',        park: 'surface', far: 1.5, cov: 40 },
  { key: 'u-ind', g: '建筑用途', label: '工业 双面对流 Cross-dock', parcel: 'big',  use: 'industrial',   park: 'surface', far: 0.6, cov: 48, dock: 'cross' },
  { key: 'u-ind2',g: '建筑用途', label: '物流 单面 Single-dock',  parcel: 'big',    use: 'industrial',   park: 'surface', far: 0.6, cov: 48, dock: 'single' },
  { key: 'u-dc',  g: '建筑用途', label: '资料中心 Data Center',   parcel: 'huge',   use: 'datacenter',   park: 'surface', far: 1.0, cov: 55 },
];
function loadDemo(d) {
  if (!d) return;
  ...
  $('#zParkHint').textContent = pre.resi ? '住宅：车位 / 户。' : '商用 / 工业：车位 / 1000 SF。';
...
if ($('#ePadBal')) $('#ePadBal').onclick = () => { computeEarthwork(); $('#ePad').value = (S._ewBalance || 0).toFixed(1); computeEarthwork(); toast('整平高设为挖填平衡点（免外运）'); };
...
function enableMap(on) {
  if (on && typeof L === 'undefined') { toast('地图需要网路连线（Leaflet 未载入）'); return; }
  S.mapMode = on;
...
async function geocode(q) {
  if (!q || !S.map) { toast('请输入地址'); return; }
  toast('正在搜索…');
  try {
    // Try Amap geocoding first (better for Chinese addresses)
    const amapKey = S.amapKey || '';
    if (amapKey) {
      try {
        const ctrl = new AbortController(); const timer = setTimeout(() => ctrl.abort(), 5000);
        const r = await fetch('https://restapi.amap.com/v3/geocode/geo?address=' + encodeURIComponent(q) + '&key=' + amapKey, { signal: ctrl.signal });
        clearTimeout(timer);
        const j = await r.json();
        if (j.geocodes && j.geocodes[0]) {
          ...
          S.map.setView([lat, lng], 18); toast('已移至：' + (j.geocodes[0].formatted_address || q).slice(0, 46)); draw();
          return;
        }
      } catch (e) { /* amap failed, try nominatim */ }
    }
    // Fallback to Nominatim with timeout
    const ctrl2 = new AbortController(); const timer2 = setTimeout(() => ctrl2.abort(), 8000);
    const r = await fetch('https://nominatim.openstreetmap.org/search?format=json&limit=1&q=' + encodeURIComponent(q), { headers: { 'Accept-Language': 'zh-CN,en' }, signal: ctrl2.signal });
    clearTimeout(timer2);
    const j = await r.json();
    if (j && j[0]) {
    ...
    if (err.name === 'AbortError') toast('搜索超时，请检查网络');
    else toast('地址查询失败：' + err.message);
  }
}
/* --- Map base layers --- */
function ensureBase(key) {
  ...
  let lyr;
  if (key === 'osm') lyr = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 23, maxNativeZoom: 19, attribution: '© OpenStreetMap' });
  else if (key === 'amapSat') lyr = L.tileLayer('https://webst0{s}.is.autonavi.com/appmaptile?style=6&x={x}&y={y}&z={z}', { maxZoom: 18, subdomains: '1234', attribution: '© 高德地图' });
  else if (key === 'amapRoad') lyr = L.tileLayer('https://webrd0{s}.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=8&x={x}&y={y}&z={z}', { maxZoom: 18, subdomains: '1234', attribution: '© 高德地图' });
  else lyr = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', { maxZoom: 23, maxNativeZoom: 19, attribution: 'Tiles © Esri' });
  S.bases[key] = lyr; return lyr;
}
function setBase(key) {
  if (!S.map) return;
  ['esri', 'osm', 'amapSat', 'amapRoad'].forEach(k => { const l = S.bases && S.bases[k]; if (l && S.map.hasLayer(l)) S.map.removeLayer(l); });
  ...
  $('#tSat').classList.toggle('active', key === 'esri' || key === 'amapSat'); $('#tStreet').classList.toggle('active', key === 'osm' || key === 'amapRoad');
}
function setTile(sat) { setBase(sat ? 'esri' : 'osm'); }   // 卫星/街道 quick buttons
function setOverlay(layer, on) { /* overlays removed in China version */ }
$('#btnTwLayers').onclick = () => {
  if (typeof L === 'undefined') { toast('地图需要网路连线'); return; }
...
$('#btnBgImg').onclick = () => $('#bgImgInput').click();
$('#bgImgInput').addEventListener('change', e => {
  ...
  reader.onload = ev => {
    ...
    img.onload = () => {
      ...
      const maxW = cv._w * 0.8, maxH = cv._h * 0.8;
      const scale = Math.min(maxW / img.width, maxH / img.height, 1);
      const w = img.width * scale, h = img.height * scale;
      S.bgImg = { img, x: (cv._w - w) / 2, y: (cv._h - h) / 2, w, h, opacity: 0.5, _dataUrl: ev.target.result };
      draw();
      toast('底图已导入 — 拖动移动，滚轮缩放，右键调整透明度');
    };
...
// bgImg: drag to move, wheel to resize, right-click to toggle opacity
cv.addEventListener('mousedown', e => {
  if (!S.bgImg || e.button !== 0) return;
  const r = cv.getBoundingClientRect(), mx = e.clientX - r.left, my = e.clientY - r.top;
  const bi = S.bgImg;
  if (mx >= bi.x && mx <= bi.x + bi.w && my >= bi.y && my <= bi.y + bi.h) {
    S.bgImgDrag = { startX: mx, startY: my, origX: bi.x, origY: bi.y };
    e.preventDefault();
  }
});
cv.addEventListener('mousemove', e => {
  if (!S.bgImgDrag) return;
  const r = cv.getBoundingClientRect(), mx = e.clientX - r.left, my = e.clientY - r.top;
  S.bgImg.x = S.bgImgDrag.origX + (mx - S.bgImgDrag.startX);
  S.bgImg.y = S.bgImgDrag.origY + (my - S.bgImgDrag.startY);
  draw();
});
cv.addEventListener('mouseup', () => { S.bgImgDrag = null; });
cv.addEventListener('wheel', e => {
  if (!S.bgImg) return;
  const r = cv.getBoundingClientRect(), mx = e.clientX - r.left, my = e.clientY - r.top;
  const bi = S.bgImg;
  if (mx < bi.x || mx > bi.x + bi.w || my < bi.y || my > bi.y + bi.h) return;
  e.preventDefault();
  const factor = e.deltaY < 0 ? 1.1 : 0.9;
  const newW = bi.w * factor, newH = bi.h * factor;
  bi.x = mx - (mx - bi.x) * factor;
  bi.y = my - (my - bi.y) * factor;
  bi.w = newW; bi.h = newH;
  draw();
}, { passive: false });
cv.addEventListener('contextmenu', e => {
  ...
  if (mx >= bi.x && mx <= bi.x + bi.w && my >= bi.y && my <= bi.y + bi.h) {
    e.preventDefault();
    bi.opacity = bi.opacity >= 0.9 ? 0.2 : Math.min(bi.opacity + 0.2, 1);
    draw();
    toast('底图透明度: ' + Math.round(bi.opacity * 100) + '%');
  }
});
$('#btnFlow').onclick = () => toggleLayer('flow', 'vis');   // 动线体检 is a toggleable layer (also in the object tree)
$('#btnMetes').onclick = openMetesModal;
$('#btnContext').onclick = fetchContextBuildings;
$('#btnCloud').onclick = openCloudModal;
$('#panelToggle').onclick = () => { const open = $('#panel').classList.toggle('open'); $('#panelToggle').innerHTML = open ? '✕ 收起面板' : '⚙ 参数'; };
...
$('#mapTrace').onclick = () => { setTool('boundary'); toast('在地图上点出基地各角，按 Enter 或点回起点收尾'); };
...
$('#entPopup').querySelectorAll('button').forEach(b => b.addEventListener('mousedown', e => {
  ...
  if (b.dataset.t === 'del') {
    ...
    hideEntPopup(); draw(); resolveActive(); toast('已删除出入口'); return;
  }
  S.selEntrance.type = b.dataset.t;
  showEntPopup(S.selEntrance); draw(); resolveActive();
  toast('出入口：' + ({ in: '只进 ▸', out: '只出 ◂', inout: '进出 ⇄' })[b.dataset.t]);
}));
...
function openMetesModal() {
  openModal('📐 测量描述 Metes & Bounds', `
    <div class="hint" style="margin-bottom:6px">每行一段：方位角 + 距离（呎）。例：<code>N45E 120</code>、<code>S 30°15' W 80.5</code>。从起点顺描述自动接成地界。</div>
    ...
    <button class="btn primary" id="mbGen" style="width:100%;justify-content:center;margin-top:8px">依描述产生地界</button>
    <div class="hint" id="mbErr" style="color:#fca5a5;margin-top:6px"></div>`);
  $('#mbGen').onclick = () => {
    const pts = parseMetes($('#mbText').value);
    if (pts.length < 3) { $('#mbErr').textContent = '至少 3 段才能组成地界（格式：N45E 100，每行一段）。'; return; }
...
function renderGenGrid() {
  ...
  $('#genGrid').innerHTML = _genSorted.map((o, i) => `<div class="optcard ${i === 0 ? 'best' : ''}">
      <div class="rank">${i === 0 ? '★ 最佳' : '#' + (i + 1)}${o.pass === false ? ' · ⚠ 停车不足' : ''}</div>
    ...
    || '<div class="hint">没有符合条件的方案。</div>';
  $('#genGrid').querySelectorAll('.apply').forEach(b => b.onclick = () => applyOption(_genSorted[+b.dataset.i]));
}
function generateOptions() {
  readParams();
  if (S.boundary.length < 3) { toast('请先画出基地边界'); return; }
  $('#busy').classList.add('show'); $('#busyTxt').textContent = '产生多个方案中…';
  setTimeout(() => {
    const out = [];
    if (S.mode === 'site') {
      ...
      const parkCfgs = [
        { name: '地面停车', park: 'surface' },
        { name: '车库地上3层', park: 'structured', above: 3, below: 0 },
        { name: '车库地上2+地下2', park: 'structured', above: 2, below: 2 },
      ];
      for (const b of biases) for (const pc of parkCfgs) for (const a of [90, 60]) {
        ...
        if (sol) out.push({
          ...
          big: sol.residential ? sol.units + ' 户' : Math.round(U.A(sol.gfa)).toLocaleString() + ' ' + U.au(),
      ...
      for (const o of ['auto', '0', '90', 'edge']) for (const a of [90, 60, 45]) {
        ...
        out.push({
          kind: 'parking', orient: o, angle: a, kpi: sol.count, k_count: sol.count, big: sol.count + ' 车位', thumb: thumbFor(sol),
          sub: `${({ auto: '自动最佳', 0: '水平 0°', 90: '垂直 90°', edge: '沿最长边' })[o]}<br>停车角度 ${a}°<br>配置 ${sol.metrics.bestAngleDeg}°`,
        });
      }
    }
    _genRaw = out;
    $('#busy').classList.remove('show');
    if (!_genRaw.length) { toast('无法产生方案'); return; }
    // custom KPI sorting (TestFit-style): pick which KPI ranks the options
    const sorts = S.mode === 'site'
      ? [['k_park', '提供车位'], ['k_yield', 'Yield 报酬'], ['k_units', '户数']]
      : [['k_count', '车位数']];
    const sel = `<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;font-size:12px;color:var(--muted);">
        排序依据 KPI：<select id="genSort" style="background:#0f1a2b;border:1px solid var(--line);color:var(--text);border-radius:6px;padding:5px 8px;">
        ${sorts.map(s => `<option value="${s[0]}">${s[1]}</option>`).join('')}</select>
        <label style="margin-left:auto;display:flex;align-items:center;gap:5px;"><input type="checkbox" id="genPass"> 只看停车达标</label></div>`;
    openModal(`产生方案 — ${_genRaw.length} 个（可自选 KPI 排序）`, sel + '<div class="optgrid" id="genGrid"></div>');
    renderGenGrid();
...
function exportReport() {
  ...
  let title, rows = [], comp = '';
  if (S.mode === 'site' && S.site) {
    const s = S.site; title = '建案可行性报告 Site Feasibility';
    rows = [['用途 Use', $('#sUse').selectedOptions[0].text], ['楼层 Floors', s.floors + ' F'],
      ['建筑高度 Height', Math.round(U.L(s.height)) + ' ' + U.lu()], ['总楼地板 GFA', fmtA(s.gfa)], ['可租 NRSF', fmtA(s.nrsf)],
      ['容积率 FAR', s.far.toFixed(2)], ['建蔽率 Coverage', s.coverage.toFixed(0) + '%']];
    if (s.residential) rows.push(['户数 Units', s.units], ['密度 Density', (s.units / s.acres).toFixed(1) + ' DU/ac']);
    rows.push(['停车 供/需 Parking', `${s.parkingProvided} / ${s.parkingRequired}`],
    ...
    comp = '<h3>法规检核 Zoning Compliance</h3><table>' +
      s.compliance.map(c => `<tr><td>${esc(c.k)}</td><td>${c.ok ? '✅ 通过' : '❌ 不符'}</td><td>${esc(c.val)}</td></tr>`).join('') + '</table>';
  } else if (S.solution) {
    const sol = S.solution; title = '停车场可行性报告 Parking Feasibility';
    const counts = { standard: 0, compact: 0, ada: 0, ev: 0, trailer: 0 }; sol.stalls.forEach(s => counts[s.type]++);
    rows = [['总车位 Total Stalls', sol.stalls.length], ['标准 Standard', counts.standard], ['⚡ EV', counts.ev],
      ['♿ ADA', counts.ada], ['配置角度 Angle', sol.metrics.bestAngleDeg + '°'],
      ['基地面积 Area', `${fmtA(area)} (${U.big(area).toFixed(2)} ${U.bu()})`]];
    if (S.opts.target > 0) rows.push(['🎯 目标 Target', `${sol.stalls.length} / ${S.opts.target} ${sol.stalls.length >= S.opts.target ? '✅' : '❌'}`]);
  } else { toast('尚无结果可做报告'); return; }
...
<h1>${esc(title)}</h1>
<div class="meta">地点：${esc($('#addrInput') ? $('#addrInput').value : '')} ｜ 日期：${date} ｜ 面积：${fmtA(area)} (${U.big(area).toFixed(2)} ${U.bu()})</div>
<img src="${img}"><table>${rowsHtml}</table>${comp}
<div class="foot">由 TestFit Clone 产生 · 概念性可行性估算，非正式工程／法律文件</div>
<button class="noprint" onclick="window.print()" style="margin-top:8px;padding:9px 18px;font-size:13px;cursor:pointer;">🖨️ 列印 / 存成 PDF</button>
  ...
  if (!w) { toast('请允许弹出视窗以开启报告'); URL.revokeObjectURL(url); return; }
  setTimeout(() => URL.revokeObjectURL(url), 60000);
  toast('报告已开启 — 用浏览器「列印 → 存成 PDF」');
}

/* side-by-side scheme compare */
function compareSchemes() {
  const arr = getSchemes();
  if (arr.length < 2) { toast('至少要 2 个已储存方案才能比较'); return; }
  const metric = sc => {
    const d = sc.data || {};
    if (sc.mode === 'site' && d.site) {
      const s = d.site;
      return { '类型': '🏢 建案', '车位数': '—', '户数/GFA': s.residential ? s.units + ' 户' : Math.round(s.gfa).toLocaleString() + ' SF',
        FAR: s.far.toFixed(2), '楼层': s.floors + 'F', '建蔽率': s.coverage.toFixed(0) + '%',
        '停车 供/需': `${s.parkingProvided}/${s.parkingRequired}`, Yield: s.fin.yieldOnCost.toFixed(1) + '%' };
    }
    const sol = d.solution;
    return { '类型': '🅿️ 停车', '车位数': sol ? sol.stalls.length : (sc.summary || '—'),
      '户数/GFA': '—', FAR: '—', '楼层': '—', '建蔽率': '—',
      '停车 供/需': '—', Yield: '—', '配置角度': sol && sol.metrics ? sol.metrics.bestAngleDeg + '°' : '—' };
  };
  const cols = arr.map(metric);
  const keys = ['类型', '车位数', '配置角度', '户数/GFA', 'FAR', '楼层', '建蔽率', '停车 供/需', 'Yield'];
  let html = '<table class="cmptable"><tr><th>项目</th>' + arr.map(sc => `<th>${esc((sc.mode === 'site' ? '🏢 ' : '🅿️ ') + sc.name)}</th>`).join('') + '</tr>';
  for (const k of keys) {
  ...
  openModal('方案比较（' + arr.length + ' 个）', html);
}
...
function buildSensitivityHTML(s) {
  ...
  let h = '<h3 style="font-size:13px;margin:16px 0 6px;color:var(--accent);">敏感度 — IRR（直栏＝租金；横列＝营造成本）</h3>';
  ...
  return h + '</table><div style="font-size:11px;color:#94a3b8;margin-top:6px;">中央蓝格＝目前方案；未杠杆 IRR，持有 ' + s.fin.holdYears + ' 年。</div>';
}
...
function takeoffRows() {
  ...
  if (S.mode === 'site' && S.site) {
    ...
    rows.push(['可建范围 Buildable', fmtA(PS.polyArea(s.envelope))]);
    rows.push(['建筑占地 Footprint', fmtA(PS.polyArea(s.footprint))]);
    rows.push(['楼层 Floors', s.floors]);
    rows.push(['总楼地板 GFA', fmtA(s.gfa)]);
    rows.push(['可租/售 NRSF', fmtA(s.nrsf)]);
    rows.push(['得房率 Efficiency', Math.round(s.nrsf / s.gfa * 100) + '%']);
    if (s.residential) {
      hdr('单元 Unit Mix');
      s.unitsByType.forEach(u => { if (u.count) rows.push([UNIT_LABEL[u.type], `${u.count} 户 × ${Math.round(U.A(u.size))} = ${fmtA(u.count * u.size)}`]); });
      rows.push(['总户数 Total Units', s.units]);
      rows.push(['密度 Density', (s.units / s.acres).toFixed(1) + ' DU/ac']);
      let beds = 0, baths = 0;
      s.unitsByType.forEach(u => { beds += (BEDS[u.type] || 0) * u.count; baths += (BATHS[u.type] || 0) * u.count; });
      rows.push(['总卧室 Total Beds', beds.toLocaleString()]);
      rows.push(['总卫浴 Total Baths', baths.toLocaleString()]);
    }
    hdr('停车 Parking');
    rows.push(['提供 Provided', s.parkingProvided]);
    rows.push(['需求 Required', s.parkingRequired]);
    rows.push(['达标', s.parkingProvided >= s.parkingRequired ? '✅' : '❌ 差 ' + (s.parkingRequired - s.parkingProvided)]);
    hdr('成本 Cost');
    rows.push(['土地 Land', '$' + Math.round(s.fin.landCost).toLocaleString()]);
    rows.push(['营造 Hard', '$' + Math.round(s.fin.hard).toLocaleString()]);
    rows.push(['软成本 Soft', '$' + Math.round(s.fin.soft).toLocaleString()]);
    rows.push(['总成本 Total', '$' + Math.round(s.fin.totalCost).toLocaleString()]);
    hdr('收益 Revenue');
    ...
    hdr('投资报酬 Returns');
    rows.push([`退场价值 Exit Value (${$('#fExitCap').value}% cap)`, '$' + Math.round(s.fin.exitValue).toLocaleString()]);
    rows.push([`IRR · ${s.fin.holdYears} 年未杠杆`, s.fin.irr == null ? '—' : s.fin.irr.toFixed(1) + '%']);
    rows.push(['权益倍数 Equity Multiple', s.fin.equityMultiple.toFixed(2) + 'x']);
  } else if (S.solution) {
    const sol = S.solution, cc = { standard: 0, compact: 0, ev: 0, ada: 0, trailer: 0 };
    sol.stalls.forEach(x => cc[x.type]++);
    const moduleArea = S.params.stallW * (S.params.stallD + S.params.aisle / 2);
    hdr('停车 Parking Takeoff');
    rows.push(['基地 Site', `${fmtA(area)} (${U.big(area).toFixed(2)} ${U.bu()})`]);
    rows.push(['总车位 Total', sol.stalls.length]);
    rows.push(['标准 Standard', cc.standard]);
    ...
    rows.push(['车道循环', S.params.oneway ? '单向 One-way' : '双向 Two-way']);
    rows.push(['单格用地 Area/Stall', fmtA(moduleArea)]);
    rows.push(['停车覆盖率 Coverage', Math.round(sol.stalls.length * moduleArea / area * 100) + '%']);
    if (S.opts.target > 0) rows.push(['🎯 目标 Target', `${sol.stalls.length} / ${S.opts.target} ${sol.stalls.length >= S.opts.target ? '✅' : '❌'}`]);
  } else return null;
  return { rows, area };
}
...
function quantityTakeoff() {
  ...
  openModal('📊 数量计算 Quantity Takeoff', html + sens);
}
...
async function exportXLSX() {
  const t = takeoffRows();
  if (!t) { toast('尚无结果可汇出'); return; }
  try { await loadVendor('vendor_xlsx.min.js'); }
  catch { toast('Excel 元件载入失败（首次需网路下载一次）'); return; }
  const XLSX = window.XLSX;
  const wb = XLSX.utils.book_new();
  const main = [['项目 Item', '数值 Value'], ...t.rows.map(r => [r[0], r[1] === '' ? '' : String(r[1])])];
  const ws = XLSX.utils.aoa_to_sheet(main); ws['!cols'] = [{ wch: 36 }, { wch: 30 }];
  XLSX.utils.book_append_sheet(wb, ws, '数量计算');
  const park = activePark();
  if (park && park.stalls.length) {
    const sa = [['#', 'type', 'x_ft', 'y_ft'], ...park.stalls.map((s, i) => [i + 1, s.type, +s.cx.toFixed(2), +s.cy.toFixed(2)])];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(sa), '车位清单');
  }
...
function autoOptimize() {
  toast('🔒 试用版不支持自动达标，请购买完整版');
}
$('#btnOptimize').onclick = () => toast('🔒 试用版不支持自动达标，请购买完整版');

$('#btnGenerate').onclick = () => toast('🔒 试用版不支持方案生成，请购买完整版');
...
function savePreset() {
  const name = ($('#presetName').value || '').trim() || `预设 ${getPresets().length + 1}`;
  const a = getPresets(); a.unshift({ name, date: new Date().toISOString().slice(0, 10), data: capturePreset() });
  setPresets(a); $('#presetName').value = ''; toast('已存成参数预设：' + name);
}
function renderPresets() {
  const list = $('#presetList'); if (!list) return; list.innerHTML = '';
  const arr = getPresets();
  if (!arr.length) { list.innerHTML = '<div class="hint">尚无参数预设。</div>'; return; }
  arr.forEach((p, i) => {
    const el = document.createElement('div'); el.className = 'scheme';
    el.innerHTML = `<div style="flex:1;overflow:hidden;"><div class="nm"></div><div class="sub"></div></div><button class="mini" title="套用">↺</button><button class="mini" title="删除">✕</button>`;
    el.querySelector('.nm').textContent = p.name;
    el.querySelector('.sub').textContent = `${(p.data.site && p.data.site.sUse) || '停车'} · ${p.unit || p.data.unit || ''} · ${p.date}`;
...
function refreshBldgPanel() {
  ...
  $('#bSel').innerHTML = S.buildings.map((bb, i) =>
    `<option value="${i}">建筑 ${i + 1}${bb.voids && bb.voids.length ? '（中庭×' + bb.voids.length + '）' : ''}</option>`).join('');
...
function updateMassInfo(b) {            // live GFA + parking-demand readout for the selected massing
  ...
  el.innerHTML = `楼地板 GFA：<b style="color:#e2e8f0">${area}</b>　(${USE_LABEL[b.use] || ''} · ${b.floors || 1} 层)<br>需要车位：<b style="color:#facc15">${req.toLocaleString()}</b> 个　(${USE_PARK[b.use] || 3}/1000SF)`;
}
...
$('#bDelete').onclick = () => {
  ...
  refreshBldgPanel(); draw(); commit(); toast('已删除建筑');
};
...
function setupCollapsible() {
  ...
  document.querySelectorAll('#panel .group').forEach((g, i) => {
    ...
    else if (!hasState && key.startsWith('图例')) g.classList.add('collapsed');   // tidy default
...
function layerCtl(key) {                       // eye + lock buttons for an object-tree row
  ...
  if (caps.vis)  h += `<span class="ot-eye${L.vis === false ? ' off' : ''}" data-lk="${key}" data-lp="vis" title="显示 / 隐藏">${L.vis === false ? '🚫' : '👁'}</span>`;
  if (caps.lock) h += `<span class="ot-lock${L.lock ? ' on' : ''}" data-lk="${key}" data-lp="lock" title="锁定（不可选取 / 编辑）">${L.lock ? '🔒' : '🔓'}</span>`;
  return h + '</span>';
}
function toggleLayer(key, prop) {
  ...
  if (key === 'flow' && prop === 'vis') {
    ...
    if (S.layers.flow.vis) { const f = S._flowCache || (S._flowCache = computeFlow()); toast(f.cells.length ? `动线体检：${f.bottlenecks} 个瓶颈卡点、${f.deadends} 个死巷端 — 越红越塞` : '请先排一次车位再体检'); }
  }
...
function buildObjTree() {
  ...
  row(1, '🗺️', '基地 Site', S.boundary.length >= 3 ? '✓' : '—', () => gotoGroup('即时数据'), 'site');
  ...
  if (S.mode === 'site') {
    row(2, '🏢', '建筑 Building', S.site ? (S.site.residential ? S.site.units + ' 户' : S.site.floors + 'F') : '—', () => { setMode('site'); gotoGroup('开发类型'); }, 'building');
    if (S.site && S.site.unitPlan) row(3, '🏠', '户型平面 Units', `${S.site.unitPlan.perFloor}/层 · ${S.layers.unitfit.vis ? '显示' : '隐藏'}`, () => toggleLayer('unitfit', 'vis'), 'unitfit');
    row(2, '🅿️', '停车 Parking', stalls, () => gotoGroup('法规 Zoning'), 'parking');
    row(2, '✅', '法规检核', S.site ? S.site.compliance.filter(c => c.ok).length + '/' + S.site.compliance.length : '—', () => gotoGroup('法规检核'));
  } else {
    row(2, '🅿️', '停车场 Surface Parking', stalls, () => { setMode('parking'); gotoGroup('停车参数'); }, 'parking');
    row(3, '🚗', '车位 Stalls', stalls, () => gotoGroup('车位种类配比'), 'parking');
    row(3, '🚪', '出入口 Access', S.entrances.length, () => setTool('entrance'), 'entrance');
  }
  if (S.obstacles.length) row(2, '⛔', '障碍/排除', S.obstacles.length, () => setTool('obstacle'), 'obstacle');
  if (S.buildings.length) row(2, '🏗️', '量体', S.buildings.length, () => setTool('building'), 'building');
  row(2, '🌳', '景观树 Trees', S.layers.trees.vis ? '显示' : '隐藏', () => toggleLayer('trees', 'vis'), 'trees');
  row(2, '🚦', '动线体检 Flow', S.layers.flow.vis ? '显示' : '隐藏', () => toggleLayer('flow', 'vis'), 'flow');
  if (S.mode === 'site') row(2, '⛰️', '挖填方 Cut/Fill', S.layers.earthwork.vis ? '显示' : '隐藏', () => toggleLayer('earthwork', 'vis'), 'earthwork');
...
function splitParcel(p, q) {
  ...
  if (base.length < 3) { toast('请先画出基地'); return; }
  ...
  if (parts.length < 2) { toast('切割线需横跨整块基地'); return; }
  ...
  if ($('#objTree') && $('#objTree').classList.contains('show')) buildObjTree();
  toast(`已切割成 ${S.parcels.length} 块子地（在物件树点选切换）`);
}
...
function loadScript(src) { return new Promise((res, rej) => { const s = document.createElement('script'); s.src = src; s.onload = res; s.onerror = () => rej(new Error('SDK 载入失败：' + src)); document.head.appendChild(s); }); }
...
async function cloudOpenShared(id) {
  try { const { db } = await cloudInit(); const doc = await db.collection('shared').doc(id).get();
    if (doc.exists) { loadCloudData(JSON.parse(doc.data().data)); toast('已载入分享的方案（唯读检视）'); }
    else toast('找不到这个分享连结'); }
  catch (e) { toast('载入分享失败：' + e.message); }
}
function openCloudModal() {
  openModal('☁️ 云端 Cloud', '<div id="cloudBody" style="min-height:120px"><div class="hint">连线中…</div></div>');
  cloudInit().then(renderCloudModal).catch(e => { const b = $('#cloudBody'); if (b) b.innerHTML = `<div class="hint" style="color:#fca5a5">云端连线失败：${esc(e.message)}<br>（请确认有网路；离线时云端功能无法使用，但工具其他部分照常）</div>`; });
}
function renderCloudModal() {
  ...
  if (!_fbUser) {
    b.innerHTML = `
      <div class="hint" style="margin-bottom:8px">登入后可把方案存到云端、跨装置开启、产生分享连结。</div>
      ...
      <input id="cem" type="email" placeholder="email" ${inp}><input id="cpw" type="password" placeholder="密码（至少 6 码）" ${inp}>
      <div class="row2" style="display:flex;gap:6px;margin-top:6px"><button class="btn" id="cin" style="flex:1;justify-content:center">登入</button><button class="btn" id="creg" style="flex:1;justify-content:center">注册新帐号</button></div>
      <div class="hint" id="cerr" style="color:#fca5a5;margin-top:6px"></div>
      <div class="hint" style="margin-top:10px;color:var(--muted)">登入后即可：存方案到云端、跨装置开启、产生唯读分享连结。</div>`;
    ...
    b.innerHTML = `
      ...
      <div class="field"><input id="cname" type="text" placeholder="方案名称" value="${esc(($('#sUse')&&S.mode==='site'?$('#sUse').selectedOptions[0].text:'停车场')+' '+new Date().toLocaleDateString())}" ${inp.replace('width:100%','flex:1')}><button class="btn primary" id="csave" style="justify-content:center">💾 存到云端</button></div>
      <button class="btn" id="cshare2" style="width:100%;justify-content:center;margin:8px 0">🔗 产生分享连结（唯读）</button>
      <div class="hint" id="clink" style="word-break:break-all;color:#7dd3fc"></div>
      <hr style="border:none;border-top:1px solid var(--line);margin:10px 0">
      <div class="hint" style="margin-bottom:4px">我的云端方案：</div>
      <div id="clist"><div class="hint">载入中…</div></div>
      <hr style="border:none;border-top:1px solid var(--line);margin:10px 0">
      <button class="btn ghost" id="cout" style="width:100%;justify-content:center">登出</button>
      <div class="hint" id="cerr" style="color:#fca5a5;margin-top:6px"></div>`;
    $('#csave').onclick = async () => { try { const { db } = await cloudInit(); await db.collection('users').doc(_fbUser.uid).collection('projects').add({ name: $('#cname').value || '未命名', data: JSON.stringify(cloudProject()), updated: Date.now() }); toast('已存到云端'); listMine(); } catch (e) { $('#cerr').textContent = e.message; } };
    $('#cshare2').onclick = async () => { try { $('#clink').textContent = '产生中…'; const url = await cloudShareLink(); $('#clink').innerHTML = `分享连结：<a href="${esc(url)}" target="_blank" style="color:#7dd3fc">${esc(url)}</a>`; try { await navigator.clipboard.writeText(url); toast('分享连结已复制'); } catch (e) {} } catch (e) { $('#cerr').textContent = e.message; } };
...
async function listMine() {
  ...
  try { const { db } = await cloudInit();
    ...
    if (snap.empty) { el.innerHTML = '<div class="hint">（还没有存过方案）</div>'; return; }
    el.innerHTML = snap.docs.map(d => { const x = d.data(); return `<div class="field" style="gap:6px"><span style="flex:1;font-size:13px">${esc(x.name || '未命名')}</span><button class="btn ghost cload" data-id="${d.id}" style="padding:3px 8px">开启</button><button class="btn ghost cdel" data-id="${d.id}" style="padding:3px 8px">删</button></div>`; }).join('');
    el.querySelectorAll('.cload').forEach(bn => bn.onclick = async () => { const { db } = await cloudInit(); const doc = await db.collection('users').doc(_fbUser.uid).collection('projects').doc(bn.dataset.id).get(); if (doc.exists) { loadCloudData(JSON.parse(doc.data().data)); closeModal(); toast('已从云端开启'); } });
...
function boot() {
  ...
  $('#regionSel').value = 'cn';       // default region = China (metric + GB dims)
  applyRegion('cn');
